...
namespace frevo
{
    GridMap::GridMap(ModelPosition* pos, int robot)
    {
        ...
        x_dim = 1 + 2 * (SENSOR_RANGE);
        y_dim = 1 + 2 * (SENSOR_RANGE);
        x_offset = SENSOR_RANGE - pose.x;
        y_offset = SENSOR_RANGE - pose.y;
        x_min = pose.x - (SENSOR_RANGE);
        x_max = pose.x + (SENSOR_RANGE);
        y_min = pose.y - (SENSOR_RANGE);
        y_max = pose.y + (SENSOR_RANGE);
    ...
    GridMap* GridMap::Clear(Pose pos, vector<meters_t> scan)
    {
        ...
        for(int a=ra-SENSOR_FOV/2; a<=ra+SENSOR_FOV; a+=SENSOR_RES){
            // iterate until laser range or obstacle
            for(int r=0; r<SENSOR_RANGE; ++r){
    ...
    vector< vector <int> > GridMap::ReachableFrontierClusters(int range)
    {
        ...
        cluster_frontiers.reserve(int(ceil(double(frontiers.size())/2.0/SENSOR_RANGE)));
        ...
        vector< vector<int> > cluster;
        cluster.reserve(2*SENSOR_RANGE);
            ...
            if(cit->size() > 2*SENSOR_RANGE){
                while(cit->size() > 0){
                    ...
                    while(d > 0){
                        ...
                        if(cluster.size() >= 2*SENSOR_RANGE){
                            break;
                        }
