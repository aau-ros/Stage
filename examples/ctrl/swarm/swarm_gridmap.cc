...
namespace swarm
{
    ...
    GridMap* GridMap::Clear(Pose pos, vector<meters_t> scan)
    {
        ...
        for(int a=ra-180; a<=ra+180; ++a){ // TODO: read fov from sensor
    ...
    {
        // check if valid raster existent
    ...
    uint8_t* GridMap::Output()
    {
        // check if valid output raster existent
        if(output_valid == false){
            // new output raster needs to be created
            output = new uint8_t[x_dim*y_dim];
            int idx = 0;
            
            if(InArray(robot, DEBUG_ROBOTS, sizeof(DEBUG_ROBOTS)/sizeof(robot)))
                printf("[%s:%d] [robot %d]: create raster of size (%d,%d)\n", StripPath(__FILE__), __LINE__, robot, x_dim, y_dim);
